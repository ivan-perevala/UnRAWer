  ...
  try {
    ...
    auto check = [&parsed](const std::string &section, const std::string &key) {
      ...
      if (ts == parsed[section].as_table().end()) {
        LOG(error) << "Warning: [" << section.c_str() << "] section does not contain \"" << key.c_str() << "\" key."
                   << std::endl;
        return false;
      }
      return true;
    ...
    // Global
    if (!check("Global", "Console"))
      return false;
    settings.conEnable = parsed["Global"]["Console"].as_boolean();

    if (!check("Global", "Threads"))
      return false;
    settings.numThreads = parsed["Global"]["Threads"].as_integer();
    if (!check("Global", "ThredsMult"))
      return false;
    settings.mltThreads = parsed["Global"]["ThredsMult"].as_floating();
    if (settings.mltThreads <= 0.0f) {
      LOG(error) << "Error parsing settings file: [Global] section: \"ThredsMult\" key value should be more than 0."
                 << std::endl;
      return false;
    }

    if (!check("Global", "ExportSubf"))
      return false;
    settings.useSbFldr = parsed["Global"]["ExportSubf"].as_boolean();

    if (!check("Global", "PathPrefix"))
      return false;
    settings.pathPrefix = parsed["Global"]["PathPrefix"].as_string();
    if (!isValidPath(settings.pathPrefix)) {
      LOG(error)
          << "Error parsing settings file: [Global] section: \"PathPrefix\" key value contains invalid characters."
          << std::endl;
    }

    if (!check("Global", "Verbosity"))
      return false;
    settings.verbosity = parsed["Global"]["Verbosity"].as_integer();
    if (settings.verbosity < 0 || settings.verbosity > 5) {
      LOG(error) << "Error parsing settings file: [Global] section: \"Verbosity\" key value is out of range."
                 << std::endl;
      return false;
    }

    // Range
    if (!check("Range", "RangeMode"))
      return false;
    settings.rangeMode = parsed["Range"]["RangeMode"].as_integer();
    if (settings.rangeMode < 0 || settings.rangeMode > 3) {
      LOG(error) << "Error parsing settings file: [Range] section: \"RangeMode\" key value is out of range."
                 << std::endl;
      return false;
    }

    // Export
    if (!check("Export", "DefaultFormat"))
      return false;
    settings.defFormat = parsed["Export"]["DefaultFormat"].as_integer();
    if (settings.defFormat < 0 || settings.defFormat > 5) {
      LOG(error) << "Error parsing settings file: [Export] section: \"DefaultFormat\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("Export", "FileFormat"))
      return false;
    settings.fileFormat = parsed["Export"]["FileFormat"].as_integer(); // .value_or(-1);
    if (settings.fileFormat < -1 || settings.fileFormat > 5) {
      LOG(error) << "Error parsing settings file: [Export] section: \"FileFormat\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("Export", "DefaultBit"))
      return false;
    settings.defBDepth = parsed["Export"]["DefaultBit"].as_integer();
    ;
    if (settings.defBDepth < 0 || settings.defBDepth > 6) {
      LOG(error) << "Error parsing settings file: [Export] section: \"DefaultBit\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("Export", "BitDepth"))
      return false;
    settings.bitDepth = parsed["Export"]["BitDepth"].as_integer(); //.value_or(-1);
    if (settings.bitDepth < -1 || settings.bitDepth > 6) {
      LOG(error) << "Error parsing settings file: [Export] section: \"BitDepth\" key value is out of range."
                 << std::endl;
      return false;
    }
    // CameraRaw
    if (!check("CameraRaw", "RawRotation"))
      return false;
    settings.rawRot = parsed["CameraRaw"]["RawRotation"].as_integer();
    if (settings.rawRot < -1 || settings.rawRot > 6) {
      LOG(error) << "Error parsing settings file: [CameraRaw] section: \"RawRotation\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("CameraRaw", "RawColorSpace"))
      return false;
    settings.rawSpace = parsed["CameraRaw"]["RawColorSpace"].as_integer();
    if (settings.rawSpace < 0 || settings.rawSpace > 10) {
      LOG(error) << "Error parsing settings file: [CameraRaw] section: \"RawColorSpace\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("CameraRaw", "Demosaic"))
      return false;
    settings.dDemosaic = parsed["CameraRaw"]["Demosaic"].as_integer();
    if (settings.dDemosaic < -2 || settings.dDemosaic > 12) {
      LOG(error) << "Error parsing settings file: [CameraRaw] section: \"Demosaic\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("CameraRaw", "half_size"))
      return false;
    settings.rawParms.half_size = static_cast<int>(parsed["CameraRaw"]["half_size"].as_boolean());
    if (!check("CameraRaw", "use_auto_wb"))
      return false;
    settings.rawParms.use_auto_wb = static_cast<int>(parsed["CameraRaw"]["use_auto_wb"].as_boolean());
    if (!check("CameraRaw", "use_camera_wb"))
      return false;
    settings.rawParms.use_camera_wb = static_cast<int>(parsed["CameraRaw"]["use_camera_wb"].as_boolean());
    if (!check("CameraRaw", "use_camera_matrix"))
      return false;
    ...
    if (settings.rawParms.use_camera_matrix < 0 || settings.rawParms.use_camera_matrix > 3) {
      LOG(error) << "Error parsing settings file: [CameraRaw] section: \"use_camera_matrix\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("CameraRaw", "highlights"))
      return false;
    ...
    if (settings.rawParms.highlight < 0 || settings.rawParms.highlight > 9) {
      LOG(error) << "Error parsing settings file: [CameraRaw] section: \"highlight\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("CameraRaw", "aberrations"))
      return false;
    ...
    if (settings.rawParms.aber[0] < 0.0f || settings.rawParms.aber[1] < 0.0f) {
      LOG(error)
          << "Error parsing settings file: [CameraRaw] section: \"aberrations\" key value should use positive floats"
          << std::endl;
      return false;
    }
    if (!check("CameraRaw", "denoise_mode"))
      return false;
    settings.denoise_mode = parsed["CameraRaw"]["denoise_mode"].as_integer();
    if (settings.denoise_mode < 0 || settings.denoise_mode > 3) {
      LOG(error) << "Error parsing settings file: [CameraRaw] section: \"denoise_mode\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("CameraRaw", "dnz_threshold"))
      return false;
    settings.rawParms.denoise_thr = parsed["CameraRaw"]["dnz_threshold"].as_floating();
    if (settings.rawParms.denoise_thr < 0.0f) {
      LOG(error)
          << "Error parsing settings file: [CameraRaw] section: \"dnz_threshold\" key value should be positive float"
          << std::endl;
      return false;
    }
    if (!check("CameraRaw", "fbdd_noiserd"))
      return false;
    ...
    if (settings.rawParms.fbdd_noiserd < 0 || settings.rawParms.fbdd_noiserd > 3) {
      LOG(error) << "Error parsing settings file: [CameraRaw] section: \"fbdd_noiserd\" key value is out of range."
                 << std::endl;
      return false;
    }

    // OCIO
    if (!check("OCIO", "OCIO_Config"))
      return false;
    ...
    // Transform
    if (!check("Transform", "LutTransform"))
      return false;
    settings.lutMode = parsed["Transform"]["LutTransform"].as_integer(); //.value_or(-1);
    if (settings.lutMode < -1 || settings.lutMode > 1) {
      LOG(error) << "Error parsing settings file: [Transform] section: \"LutTransform\" key value is out of range."
                 << std::endl;
      return false;
    }
    ...
    if (lutDefault != "") {
      ...
      if (settings.dLutPreset == "") {
        LOG(error) << "Error parsing settings file: [Transform] section: \"LutDefault\" key value is invalid."
                   << std::endl;
        return false;
      }
    }
    // Unsharp
    if (!check("Unsharp", "sharp_mode"))
      return false;
    settings.sharp_mode = parsed["Unsharp"]["sharp_mode"].as_integer();
    if (settings.sharp_mode < -1 || settings.sharp_mode > 1) {
      LOG(error) << "Error parsing settings file: [Unsharp] section: \"sharp_mode\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("Unsharp", "sharp_kernel"))
      return false;
    settings.sharp_kernel = parsed["Unsharp"]["sharp_kernel"].as_integer();
    if (settings.sharp_kernel < 0 || settings.sharp_kernel > 12) {
      LOG(error) << "Error parsing settings file: [Unsharp] section: \"sharp_kernel\" key value is out of range."
                 << std::endl;
      return false;
    }
    if (!check("Unsharp", "sharp_width"))
      return false;
    settings.sharp_width = parsed["Unsharp"]["sharp_width"].as_floating();
    if (settings.sharp_width < 0.0f) {
      LOG(error) << "Error parsing settings file: [Unsharp] section: \"sharp_width\" key value should be positive float"
                 << std::endl;
    }
    if (!check("Unsharp", "sharp_contrast"))
      return false;
    settings.sharp_contrast = parsed["Unsharp"]["sharp_contrast"].as_floating();
    if (settings.sharp_contrast < 0.0f) {
      LOG(error)
          << "Error parsing settings file: [Unsharp] section: \"sharp_contrast\" key value should be positive float"
          << std::endl;
      return false;
    }
    if (!check("Unsharp", "sharp_treshold"))
      return false;
    settings.sharp_tresh = parsed["Unsharp"]["sharp_treshold"].as_floating();
    if (settings.sharp_tresh < 0.0f) {
      LOG(error)
          << "Error parsing settings file: [Unsharp] section: \"sharp_treshold\" key value should bepositive float"
          << std::endl;
    }
...
void printSettings(Settings &settings) {
  ...
  qDebug() << qPrintable(
      QString("Half -size raw image: %1").arg(settings.rawParms.half_size == 0 ? "disabled" : "enabled"));
  qDebug() << qPrintable(
      QString("Use auto white balance: %1").arg(settings.rawParms.use_auto_wb == 0 ? "disabled" : "enabled"));
  qDebug() << qPrintable(
      QString("Use camera white balance: %1").arg(settings.rawParms.use_camera_wb == 0 ? "disabled" : "enabled"));
